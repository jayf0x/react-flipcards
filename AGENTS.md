...
| `src/FlipCard.tsx`            | A single 3D card (`mode` `sync`/`queue`). Chase-latest loop: stores only the displayed face and flips toward the latest `value`, re-reading it on every `transitionEnd` (`sync` jumps, `queue` steps +1 mod 10). The animation lives here. |
| `src/OdometerCard.tsx`        | The `mode='spin'` renderer: a vertical 0–9 strip scrolled by one `translateY` transition (lands in one `duration`). Chase-latest like `FlipCard`. The panel picks it over `FlipCard` when `mode==='spin'`.                                 |
